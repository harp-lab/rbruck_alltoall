	...
	for (int n = 2; n <= 512; n = n * 2) {
		...
		for (int i = 0; i < basecount; i++) {
			for (int it=0; it < loopcount; it++) {
				...
				double st = MPI_Wtime();
				intra_communication_test(ncores, bases[i], (char*)send_buffer, n, MPI_UNSIGNED_LONG_LONG, (char*)recv_buffer, n, MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD);
				double et = MPI_Wtime();
...
//					std::cout << "[GourpRbruck] " << bases[i] << " has errors" << std::endl;
				...
				if (warmup == 0) {
					...
					if (total_time == max_time)
						std::cout << "[Intra-Comm] " << nprocs << ", " << n << ", " << bases[i] << ", " << max_time << std::endl;
				}
			}
		}
		...
		for (int i = 0; i < basecount; i++) {
			for (int it=0; it < loopcount; it++) {
				...
				double st = MPI_Wtime();
				inter_communication_test(ncores, bases[i], (char*)send_buffer, n, MPI_UNSIGNED_LONG_LONG, (char*)recv_buffer, n, MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD);
				double et = MPI_Wtime();
				double total_time = et - st;
...
//					std::cout << "[GourpRbruck] " << bases[i] << " has errors" << std::endl;
				...
				if (warmup == 0) {
					...
					if (total_time == max_time)
						std::cout << "[Inter-Comm] " << nprocs << ", " << n << ", " << bases[i] << ", " << max_time << std::endl;
				}
			}
		}
